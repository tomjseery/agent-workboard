...
/// The provider-owned environment variable that redirects a launch to its bundle.
...
/// Where the provider writes its own transcripts inside a bundle. Retained after close so
/// `session resume` and `recover` keep working.
...
/// The provider credential the child needs to stay logged in. Linked in at launch, unlinked at
/// close, never read into Workboard state.
...
/// Provider files carried into the bundle by reference so the user keeps their own model,
/// approval, and policy settings. Never Workboard-owned.
...
pub fn prepare_bundle(
    ...
    // A retired-then-reconstructed bundle reuses its root, so start from a clean capability
    // subtree rather than merging with whatever an interrupted launch left behind.
    retire_bundle(root)?;
    ...
    for name in referenced_files(context.tool) {
        // Optional: a provider with no such file simply has nothing to carry across.
...
/// Removes only the capability subtree. The provider's own transcripts stay, so a closed session
/// remains resumable and recoverable while its skills, hooks, and credential link do not survive.
...
/// Hard-links so the child reads the provider's own credential without a copy that outlives the
/// session. Falls back to a copy only across volumes, which the close path removes either way.

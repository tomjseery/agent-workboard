...
/// Everything the launch-scoped capability bundle needs that Workboard's own environment owns
/// rather than the hierarchy: where bundles live, where the provider keeps its real home, and
/// which executable and database the injected hooks and skills must address.
...
mod tests {
    ...
    fn a_managed_launch_injects_only_its_role_bundle_and_never_the_provider_home() {
        ...
        // The Work-item execution bundle, and nothing from another role.
        ...
        // A normally opened CLI reads the provider's own home, which stays untouched.
        ...
        // Only the child receives the scoped token, role, owner, repository, and bundle path.
        ...
        // The parent process running Workboard has none of it.
    ...
    /// Managed launches always materialise a bundle, so fixtures supply a provider home with a
    /// credential and a bundle parent under the temporary directory.

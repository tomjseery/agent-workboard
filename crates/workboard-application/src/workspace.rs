...
impl WorkboardApplication {
    ...
    /// Every capability-bundle transcript directory recorded for this provider. A managed session
    /// writes its transcripts inside its own bundle, so discovery must sweep these alongside the
    /// provider's default home or resume and recovery would not see managed sessions at all.

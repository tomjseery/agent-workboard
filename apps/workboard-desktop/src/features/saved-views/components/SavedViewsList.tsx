...
export function SavedViewsList({ workspaceId }: { workspaceId: WorkspaceId }) {
  ...
  return <ul className="grid gap-2">{state.views.map((view) => <li key={view.id}><Link to="/workspaces/$workspaceId/views/$viewId" params={{ workspaceId, viewId: view.id }} className="block rounded-lg border border-[var(--border)] px-3 py-2">{view.title}</Link></li>)}</ul>;
}

...
export function WorkItemDetail({ workspaceId, workItemId }: { workspaceId: WorkspaceId; workItemId: WorkItemId }) {
  ...
  return (
    <article className="min-w-0 space-y-6">
      ...
      <section id="dependencies" tabIndex={-1} aria-labelledby="dependencies-title" className="scroll-mt-6 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 id="dependencies-title" className="text-lg font-semibold">Dependencies and blockers</h2><p className="mt-2">Readiness: {detail.dependencyReadiness.replaceAll("_", " ")}</p>{detail.blockers.length === 0 ? <p className="mt-2 text-[var(--muted-text)]">No authoritative blockers are recorded.</p> : <ul className="mt-3 space-y-2">{detail.blockers.map((blocker, index) => <li key={`${blocker.code}:${index}`} role="alert" className="rounded-lg border border-[var(--warning-muted)] p-3"><p>{blocker.message}</p>{blocker.prerequisite != null && <a href={`/workspaces/${workspaceId}/work-items/${blocker.prerequisite.id}?q=`} className="mt-2 inline-block text-sm underline">Open {blocker.prerequisite.title}</a>}</li>)}</ul>}</section>
      ...
      <section id="checkpoints" tabIndex={-1} aria-labelledby="checkpoints-title" className="scroll-mt-6 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 id="checkpoints-title" className="text-lg font-semibold">Checkpoint history</h2>{detail.checkpointHistory.length === 0 ? <p className="mt-2">No checkpoints are recorded.</p> : <ol className="mt-3 space-y-3">{detail.checkpointHistory.map((checkpoint) => <li key={checkpoint.id} className="rounded-lg border border-[var(--border)] p-3"><div className="flex flex-wrap justify-between gap-2 text-sm"><strong>{checkpoint.nextAction.replaceAll("_", " ")}</strong><span>{checkpoint.recordedAt}</span></div><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words font-sans text-sm">{checkpoint.summary}</pre><a href={`/workspaces/${workspaceId}/sessions/${checkpoint.sessionId}`} className="mt-2 inline-block text-sm underline">Open recording session</a></li>)}</ol>}</section>
      <section id="resources" tabIndex={-1} aria-labelledby="resources-title" className="scroll-mt-6 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 id="resources-title" className="text-lg font-semibold">Repositories and checkouts</h2><ul className="mt-3 flex flex-wrap gap-2">{detail.repositories.map((repository) => <li key={repository.id}><a href={`/workspaces/${workspaceId}/repositories/${repository.id}?q=`} className="inline-block rounded-lg border border-[var(--border)] px-3 py-2">{repository.title}</a></li>)}</ul>{detail.checkouts.length === 0 ? <p className="mt-3">No effective checkouts are recorded.</p> : <ul className="mt-3 grid gap-2 sm:grid-cols-2">{detail.checkouts.map((checkout) => <li key={checkout.id}><a href={`/workspaces/${workspaceId}/checkouts/${checkout.id}`} className="block rounded-lg border border-[var(--border)] p-3"><strong>{checkout.repository.title}</strong><span className="block text-sm">{checkout.availability} · {checkout.purpose.replaceAll("_", " ")}</span></a></li>)}</ul>}</section>
      <section id="sessions" tabIndex={-1} aria-labelledby="sessions-title" className="scroll-mt-6 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 id="sessions-title" className="text-lg font-semibold">Sessions</h2>{detail.sessions.length === 0 ? <p className="mt-2">No bound sessions are recorded.</p> : <ul className="mt-3 grid gap-2 sm:grid-cols-2">{detail.sessions.map((session) => <li key={session.id}><a href={`/workspaces/${workspaceId}/sessions/${session.id}`} className="block rounded-lg border border-[var(--border)] p-3"><strong>{session.provider} {session.role.replaceAll("_", " ")}</strong><span className="block text-sm">{session.liveness.state.replaceAll("_", " ")}{session.liveness.stale ? " · stale evidence" : ""}</span></a></li>)}</ul>}</section>

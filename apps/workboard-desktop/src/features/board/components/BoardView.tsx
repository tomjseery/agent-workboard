...
export function BoardView({ workspaceId, evidenceLinks = false, onOpenWorkItem }: BoardViewProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {board.totalCount === 0 ? <p>No Work items match this board view.</p> : <div aria-label="Work item board" className="flex gap-4 overflow-x-auto pb-3">{board.visibleLanes.map((lane) => <VirtualLane key={lane.key} lane={lane} workspaceId={workspaceId} evidenceLinks={evidenceLinks} cards={board.cardsByLane.get(lane.key) ?? []} selectedWorkItemId={board.selectedWorkItemId} focusedWorkItemId={board.focusedWorkItemId} onSelect={(card) => board.select(card.workItem.id)} onFocus={(card) => board.focus(card.workItem.id)} onOpen={(card) => onOpenWorkItem(card.workItem.id)} onMove={board.move} />)}</div>}

...
export function useBoard(workspaceId: WorkspaceId, scope: BoardScope = {}) {
  ...
  const toggleLane = useBoardInteractionStore((state) => state.toggleLane);
  ...
  const query = useBoardQuery(workspaceId, {
    ...
    repositoryIds: scope.repositoryIds ?? filters.repositoryIds,
  ...
  return {
    ...
    isRepositoryScoped: scope.repositoryIds !== undefined,
    selectedWorkItemId,
    focusedWorkItemId,
    filters,
    select,

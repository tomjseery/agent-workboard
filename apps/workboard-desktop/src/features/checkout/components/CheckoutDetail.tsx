...
export function CheckoutDetail({ workspaceId, checkoutId }: { workspaceId: WorkspaceId; checkoutId: CheckoutId }) {
  ...
  return <div className="space-y-5">
    ...
    <section aria-labelledby="checkout-work-items"><h2 id="checkout-work-items" className="font-semibold">Work items</h2>{projection.bindings.every((binding) => binding.workItemId == null) ? <p className="mt-2">No Work-item binding is recorded.</p> : <ul className="mt-2 flex flex-wrap gap-2">{projection.bindings.flatMap((binding) => binding.workItemId == null ? [] : [binding.workItemId]).map((workItemId) => <li key={workItemId}><Link to="/workspaces/$workspaceId/work-items/$workItemId" params={{ workspaceId, workItemId }} search={{ q: "" }} className="rounded-lg border border-[var(--border)] px-3 py-2 text-sm">Work item {workItemId.slice(0, 8)}</Link></li>)}</ul>}</section>
  </div>;
}
